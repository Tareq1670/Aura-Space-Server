...
const stripe = STRIPE_SECRET_KEY
    ? new stripe_1.default(STRIPE_SECRET_KEY, { apiVersion: "2025-12-01" })
    : null;
        ...
        switch (event.type) {
            ...
            case "checkout.session.expired": {
                ...
                if (bookingId) {
                    const objectId = toObjectId(parseId(bookingId));
                    if (objectId) {
                        await bookingsCol.updateOne({ _id: objectId, status: "pending" }, {
                            $set: {
                                status: "cancelled",
                                cancelledBy: "guest",
...
ensurePayoutMethodIndexes().catch((err) => console.error("❌ Failed to create payout method indexes:", err));
// Booking indexes
...
ensureBookingIndexes().catch((err) => console.error("❌ Failed to create booking indexes:", err));
// Transaction indexes
...
ensureTransactionIndexes().catch((err) => console.error("❌ Failed to create transaction indexes:", err));
// Wishlist indexes
...
ensureWishlistIndexes().catch((err) => console.error("❌ Failed to create wishlist indexes:", err));
// Review indexes
...
ensureReviewIndexes().catch((err) => console.error("❌ Failed to create review indexes:", err));
// Message indexes
...
ensureMessageIndexes().catch((err) => console.error("❌ Failed to create message indexes:", err));
...
ensureAIIndexes().catch((err) => console.error("❌ Failed to create AI indexes:", err));
// ============================================================
// HELPERS
// ============================================================
let JWKS = null;
...
async function findUsersMap(usersCol, userIds) {
    const uniqueIds = [...new Set(userIds.map((id) => String(id).trim()))].filter(Boolean);
    if (uniqueIds.length === 0)
        return new Map();
    const objectIds = uniqueIds
        .filter((id) => mongodb_1.ObjectId.isValid(id) && id.length === 24)
        .map((id) => new mongodb_1.ObjectId(id));
    const stringIds = uniqueIds.filter((id) => !mongodb_1.ObjectId.isValid(id) || id.length !== 24);
    const conditions = [];
    ...
    const users = await usersCol
        .find({ $or: conditions })
        .toArray();
    return new Map(users.map((u) => [toIdString(u._id), u]));
}
...
async function checkDateOverlap(bookingsCol, propertyId, checkIn, checkOut, excludeBookingId) {
    const filter = {
        propertyId,
    ...
    try {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                filters: {
                    applied: Object.keys(req.query).filter((k) => !["page", "limit", "sort"].includes(k)),
                },
            },
        });
    }
...
app.post("/api/properties", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        ...
        const status = requestedStatus &&
            validDraftStatuses.includes(requestedStatus)
            ? requestedStatus
            : user.role === "admin"
                ? "active"
                : "pending";
...
app.get("/api/bookings/my-bookings", verifyToken, async (req, res) => {
    try {
        ...
        const col = db.collection("bookings");
        const { page, limit, skip } = getPagination(req.query, 50, 10);
        const filter = { guestId: toIdString(user._id) };
...
app.get("/api/bookings/host-reservations", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const user = req.user;
        const db = await getDb();
        const col = db.collection("bookings");
        const { page, limit, skip } = getPagination(req.query, 50, 10);
        const filter = { hostId: toIdString(user._id) };
        if (req.query.status && ["pending", "confirmed", "cancelled", "completed"].includes(String(req.query.status))) {
            filter.status = String(req.query.status);
        }
        ...
        const usersCol = db.collection("user");
        const usersMap = await findUsersMap(usersCol, bookings.map((b) => b.guestId));
...
app.get("/api/bookings/:id", verifyToken, async (req, res) => {
    try {
        ...
        const db = await getDb();
        const col = db.collection("bookings");
        const booking = await col.findOne({ _id: objectId });
        if (!booking) {
...
app.put("/api/bookings/:id/confirm", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
        ...
        await col.updateOne({ _id: objectId }, { $set: { status: "confirmed", updatedAt: new Date() } });
        res.status(200).json({
            success: true,
...
app.put("/api/bookings/:id/complete", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
        ...
        await col.updateOne({ _id: objectId }, { $set: { status: "completed", updatedAt: new Date() } });
        res.status(200).json({
            success: true,
...
app.get("/api/admin/bookings", verifyToken, verifyAdmin, async (req, res) => {
    try {
        ...
        if (req.query.guestId)
            filter.guestId = String(req.query.guestId);
        if (req.query.hostId)
            filter.hostId = String(req.query.hostId);
        if (req.query.propertyId)
            filter.propertyId = String(req.query.propertyId);
...
app.put("/api/admin/bookings/:id/force-cancel", verifyToken, verifyAdmin, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
...
app.get("/api/payments/payout-method", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("payout_methods");
        const method = await col.findOne({ userId: toIdString(req.user._id) });
        if (!method) {
...
app.post("/api/payments/request-payout", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const user = req.user;
...
app.get("/api/payments/payout-history", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("transactions");
        const { page, limit, skip } = getPagination(req.query, 50, 10);
...
app.get("/api/transactions/my-transactions", verifyToken, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("transactions");
        const { page, limit, skip } = getPagination(req.query, 50, 10);
...
app.get("/api/transactions/host-transactions", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("transactions");
        const { page, limit, skip } = getPagination(req.query, 50, 10);
...
app.get("/api/transactions/stats", verifyToken, async (req, res) => {
    try {
        ...
        const thisMonthSpend = thisMonthArr[0]?.total || 0;
...
// POST refund (admin)
app.post("/api/transactions/refund/:bookingId", verifyToken, verifyAdmin, async (req, res) => {
    try {
        ...
        await transactionsCol.updateOne({ _id: payment._id }, { $set: { status: "refunded" } });
        res.status(200).json({
            success: true,
...
app.post("/api/admin/payments/process-payout", verifyToken, verifyAdmin, async (req, res) => {
    try {
        ...
        const txn = await col.findOne({ transactionId });
        if (!txn) {
            res.status(404).json({ success: false, message: "Transaction not found." });
            return;
        }
        ...
        await col.updateOne({ _id: txn._id }, { $set: { status: "success" } });
        res.status(200).json({
            success: true,
...
app.get("/api/admin/revenue", verifyToken, verifyAdmin, async (req, res) => {
    try {
        const db = await getDb();
        const transactionsCol = db.collection("transactions");
        const usersCol = db.collection("user");
        ...
        const [monthlyRevenue, commissionTotalResult, pendingPayoutsResult, allSuccessPayments, allCommissions,] = await Promise.all([
        ...
        const userIds = [...new Set(allSuccessPayments.map((p) => p.userId).filter(Boolean))];
        const userIdsObj = userIds.map((id) => toObjectId(id)).filter((id) => id !== null);
        const userDocs = userIdsObj.length > 0 ? await usersCol.find({ _id: { $in: userIdsObj } }).toArray() : [];
        const userMap = new Map(userDocs.map((u) => [toIdString(u._id), u.name || u.email || "Unknown"]));
        const hostEarnings = {};
        allSuccessPayments.forEach((p) => {
            if (p.userId) {
        ...
        allCommissions.forEach((c) => {
            if (c.description) {
                ...
                if (!propEarnings[name])
                    propEarnings[name] = { name, earnings: 0 };
        ...
        const paymentTotal = allSuccessPayments.reduce((s, p) => s + p.amount, 0);
...
app.get("/api/wishlist", verifyToken, async (req, res) => {
    try {
        ...
        const userId = toIdString(req.user._id);
        const { page, limit, skip } = getPagination(req.query, 50, 12);
        const filter = { userId, listType: { $ne: "folder" } };
        if (req.query.listName)
            filter.listName = String(req.query.listName);
        const [items, total] = await Promise.all([
            col.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).toArray(),
            col.countDocuments(filter),
        ]);
        // Join property data (batched)
        const propIds = items.map((i) => parseId(i.propertyId)).filter((id) => id !== null);
        const propOids = propIds.map((id) => toObjectId(id)).filter((id) => id !== null);
        let propertyMap = new Map();
        if (propOids.length > 0) {
            const props = await propertiesCol
                .find({ _id: { $in: propOids }, status: { $ne: "deleted" } })
                .toArray();
            propertyMap = new Map(props.map((p) => [toIdString(p._id), p]));
        }
...
// GET check wishlist
app.get("/api/wishlist/check/:propertyId", verifyToken, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("wishlist");
        const userId = toIdString(req.user._id);
...
app.put("/api/wishlist/:id/list", verifyToken, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
...
app.post("/api/wishlist/lists", verifyToken, async (req, res) => {
    try {
        const { listName } = req.body;
...
// GET wishlist lists
app.get("/api/wishlist/lists", verifyToken, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("wishlist");
        const userId = toIdString(req.user._id);
...
async function updatePropertyRating(propertiesCol, propertyId) {
    try {
        ...
        const avg = stats.length > 0 ? Math.round((stats[0].avgRating || 0) * 10) / 10 : 0;
        const cnt = stats.length > 0 ? stats[0].count || 0 : 0;
        const propOid = toObjectId(parseId(propertyId));
        if (propOid) {
            await propertiesCol.updateOne({ _id: propOid }, { $set: { rating: avg, reviewCount: cnt } });
        }
    }
    catch (err) {
...
app.get("/api/reviews/property/:id", async (req, res) => {
    try {
        ...
        const usersCol = db.collection("user");
        const { page, limit, skip } = getPagination(req.query, 50, 10);
        const filter = { propertyId, isReported: { $ne: true } };
...
app.get("/api/reviews/my-reviews", verifyToken, async (req, res) => {
    try {
        ...
        const reviewBookingIds = reviews.map((r) => parseId(r.bookingId)).filter(Boolean);
        const reviewBookingOids = reviewBookingIds.map((id) => toObjectId(id)).filter((id) => id !== null);
        let reviewBookingMap = new Map();
        if (reviewBookingOids.length > 0) {
            const reviewBookings = await bookingsCol
                .find({ _id: { $in: reviewBookingOids } })
                .toArray();
            reviewBookingMap = new Map(reviewBookings.map((b) => [toIdString(b._id), b]));
        }
        const reviewsWithProperty = reviews.map((r) => {
...
// GET host reviews
app.get("/api/reviews/host-reviews", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const db = await getDb();
        const reviewsCol = db.collection("reviews");
        const propertiesCol = db.collection("properties");
        ...
        const enrichedItems = reviews;
        const guestMap = await findUsersMap(usersCol, enrichedItems.map((r) => r.guestId));
        const hostPropIds = enrichedItems
            .map((r) => parseId(r.propertyId))
            .filter((id) => id !== null);
        const hostPropOids = hostPropIds.map((id) => toObjectId(id)).filter((id) => id !== null);
        let hostPropMap = new Map();
        if (hostPropOids.length > 0) {
            const hostProps = await propertiesCol
                .find({ _id: { $in: hostPropOids } })
                .toArray();
            hostPropMap = new Map(hostProps.map((p) => [toIdString(p._id), p]));
        }
        const enriched = enrichedItems.map((r) => {
        ...
        const averageRating = avgResult.length > 0
            ? Math.round((avgResult[0].avg || 0) * 10) / 10
            : 0;
        res.status(200).json({
            success: true,
            data: {
...
app.put("/api/reviews/:id", verifyToken, async (req, res) => {
    try {
        ...
        const db = await getDb();
        const col = db.collection("reviews");
        const review = await col.findOne({ _id: objectId });
        if (!review) {
...
app.put("/api/reviews/:id/reply", verifyToken, verifyHostOrAdmin, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
        ...
        await col.updateOne({ _id: objectId }, { $set: { hostReply: String(reply).trim(), hostReplyDate: new Date() } });
...
// PUT report review
app.put("/api/reviews/:id/report", verifyToken, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
...
app.get("/api/admin/advertise/stats", verifyToken, verifyAdmin, async (_req, res) => {
    try {
        const db = await getDb();
        const usersCol = db.collection("user");
        const bookingsCol = db.collection("bookings");
        const transactionsCol = db.collection("transactions");
        const [totalUsers, totalBookings, paymentAgg,] = await Promise.all([
...
app.post("/api/admin/advertise/waitlist", verifyToken, verifyAdmin, async (req, res) => {
    try {
...
app.post("/api/conversations", verifyToken, async (req, res) => {
    try {
        ...
        const existing = await col.findOne({ participants });
        if (existing) {
...
app.post("/api/messages", verifyToken, async (req, res) => {
    try {
        ...
        const conversation = await convCol.findOne({ _id: convOid });
        if (!conversation) {
        ...
        await convCol.updateOne({ _id: convOid }, { $set: { lastMessage: content, lastMessageAt: new Date(), updatedAt: new Date() } });
...
app.get("/api/messages/unread-count", verifyToken, async (req, res) => {
    try {
        ...
        const convIds = conversations.map((c) => toIdString(c._id));
        if (convIds.length === 0) {
...
app.get("/api/messages/:conversationId", verifyToken, async (req, res) => {
    try {
        ...
        const conversation = await convCol.findOne({ _id: convOid, participants: userId });
        if (!conversation) {
            res.status(404).json({ success: false, message: "Conversation not found." });
            return;
        }
        ...
        res.status(200).json({
            success: true,
            data: messages.map((m) => ({ ...m, _id: toIdString(m._id) })),
            pagination: { total, totalPages: Math.ceil(total / limit), currentPage: page, limit },
        });
    }
    catch {
...
app.put("/api/messages/:id/read", verifyToken, async (req, res) => {
    try {
        ...
        const message = await msgCol.findOne({ _id: msgOid });
        if (!message) {
...
app.put("/api/conversations/:id/read-all", verifyToken, async (req, res) => {
    try {
        ...
        const isParticipant = await convCol.findOne({ _id: convOid, participants: userId });
        if (!isParticipant) {
            res.status(404).json({ success: false, message: "Conversation not found." });
            return;
        }
        await msgCol.updateMany({ conversationId, senderId: { $ne: userId }, isRead: false }, { $set: { isRead: true } });
...
app.get("/api/dashboard/admin", verifyToken, verifyAdmin, async (req, res) => {
    try {
        ...
        const now = new Date();
        const startOfYear = new Date(now.getFullYear(), 0, 1);
        const [totalUsers, totalProperties, totalBookings, commissionResult, pendingPayoutsResult, recentBookings, signupsResult, bookingsByStatus, propertiesByCategory, reportedReviews,] = await Promise.all([
...
app.get("/api/ai/recommendations", verifyToken, async (req, res) => {
    try {
        const groq = getGroq();
        if (!groq) {
        ...
        const reviewsCol = db.collection("reviews");
        const propertiesCol = db.collection("properties");
        const { location, budget, guests, propertyType } = req.query;
        ...
        if (propertyType)
            filter.category = propertyType.toLowerCase();
        if (guests)
            filter["details.maxGuests"] = { $gte: parseInt(guests) };
        if (budget)
            filter["price.perNight"] = { $lte: parseFloat(budget) };
        ...
        const userContext = {
            previousBookings: bookings.map((b) => ({ propertyTitle: b.propertyTitle, city: b.city || "Unknown", status: b.status })),
...
[{"propertyId":"...","title":"...","reason":"..."}]`;
        ...
        const enriched = recommendations.slice(0, 10).map((r) => {
            const prop = properties.find((p) => p._id.toString() === r.propertyId);
            if (!prop)
                return null;
            return {
...
app.post("/api/ai/chat", async (req, res) => {
    try {
        ...
        if (userId) {
            ...
            if (conversationId) {
                const oid = new mongodb_1.ObjectId(conversationId);
                conversation = await aiConvoCol.findOne({ _id: oid, userId });
            }
            if (!conversation) {
        ...
        if (userId && conversationIdOut) {
            ...
            const aiConvoCol = db.collection("ai_conversations");
            const now = new Date();
            await aiConvoCol.updateOne({ _id: new mongodb_1.ObjectId(conversationIdOut) }, {
                $push: {
                    messages: {
                        $each: [
...
- Active Bookings: ${bookingCount}`;
        ...
        const now = new Date();
        await aiConvoCol.updateOne({ _id: conversation._id }, {
            $push: {
                messages: {
                    $each: [
                        { role: "user", content: message, createdAt: now },
...
app.get("/api/ai/chat/history", verifyToken, async (req, res) => {
    try {
        ...
        if (conversationId) {
            const oid = new mongodb_1.ObjectId(conversationId);
            const convo = await aiConvoCol.findOne({ _id: oid, userId });
            if (!convo) {
                res.status(404).json({ success: false, message: "Conversation not found." });
                return;
            }
...
app.delete("/api/ai/chat/:conversationId", verifyToken, async (req, res) => {
    try {
        const userId = req.user._id.toString();
...
app.post("/api/ai/generate-description", verifyToken, async (req, res) => {
    try {
        ...
        const { title, propertyType, placeType, city, country, bedrooms, bathrooms, guests, beds, amenities, tone = "professional", length = "medium", } = req.body;
        if (!title) {
...
app.get("/api/blogs", async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            data: {
                blogs: blogs.map(buildBlogResponse),
...
app.get("/api/blogs/featured", async (_req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            data: { blogs: blogs.map(buildBlogResponse) },
        });
    }
    catch (error) {
...
app.get("/api/blogs/my/blogs", verifyToken, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("blogs");
        const userId = toIdString(req.user._id);
...
app.get("/api/blogs/:slug", async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            data: { blog: buildBlogResponse(blog) },
        });
    }
    catch (error) {
...
app.put("/api/blogs/:id", verifyToken, async (req, res) => {
    try {
        ...
        if (excerpt !== undefined)
            updates.excerpt = String(excerpt).trim().slice(0, 300);
        if (coverImage !== undefined)
            updates.coverImage = coverImage || null;
        if (tags !== undefined && Array.isArray(tags)) {
            updates.tags = tags.map((t) => String(t).trim().toLowerCase()).filter(Boolean).slice(0, 10);
        }
        ...
        res.status(200).json({
            ...
            data: buildBlogResponse(updated),
        });
    }
    catch (error) {
...
app.get("/api/admin/blogs", verifyToken, verifyAdmin, async (req, res) => {
    try {
        const db = await getDb();
        const col = db.collection("blogs");
        const { page, limit, skip } = getPagination(req.query, 50, 20);
...
app.put("/api/admin/blogs/:id/feature", verifyToken, verifyAdmin, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
...
app.delete("/api/admin/blogs/:id", verifyToken, verifyAdmin, async (req, res) => {
    try {
        const id = parseId(req.params.id);
        const objectId = toObjectId(id);
        if (!objectId) {
...
app.post("/api/ai/blog-generator", verifyToken, async (req, res) => {
    try {
        const groq = getGroq();
        if (!groq) {
            res.status(503).json({ success: false, message: "AI service not configured." });
...
- Make it engaging and valuable for readers`;
        const messages = [
            { role: "system", content: systemPrompt },
            { role: "user", content: `Write a ${selectedLength} ${selectedStyle} blog about: ${topic.trim()}` },
        ];
        const models = [AI_MODEL, ...AI_MODEL_FALLBACKS];
        let completion = null;
        for (const model of models) {
